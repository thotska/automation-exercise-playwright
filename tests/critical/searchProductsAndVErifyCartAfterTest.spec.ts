...
test.describe('Search Product Test', async () => {
    ...
    test('Search product and verify products', async ({ page }) => {
        ...
        await searchedProductsPage.clickOnAddToCart()
        await basePage.clickOnNavigationLink("Cart")
        await searchedProductsPage.verifyingProductsInCart()
        await basePage.clickOnNavigationLink("signup/login")
        await signUpLoginPage.loginWithValidEmailAndPassword(process.env.tetstuseremail!, process.env.testuserpassword!)
        await basePage.clickOnNavigationLink("Cart")
        await searchedProductsPage.verifyingProductsInCart()
    })
})  